...
var FinTSClient = function (inBlz, inKundenId, inPin, inBankenlist, logger) {
  ...
  */
  ...
  /**
  * SEPA Einzelüberweisung ausführen
  * @param {Object} debtAccount zu belastendes Konto {iban,bic,kontoNr,unterKonto,ctryCode,blz}
  * @param {Object} credAccount Empfängerkonto, enthält IBAN und optonal BIC {iban,bic}
  * @param {String} credName Name des Empfängers
  * @param {String} description Betreffzeile der Überweisung
  * @param {number} amount zu überweisender Betrag
  * @param {function} cb Callback, dient zum Überprüfen der TAN
  */
  me.MsgSEPASingleTransfer = function (debtAccount, credAccount, credName, description, amount, cb) {
    // 1. Eingangsparameter prüfen
    if (!debtAccount) {
      throw new Error('debtAccount is missing.')
    }
    if (!credAccount || !credAccount.iban) {
      throw new Error('credAccount or credAccount.iban is missing.')
    }
    if (!credName) {
      throw new Error('credName is missing.')
    }
    if (!credAccount) {
      throw new Error('credAccount is missing.')
    }
    if (!description) {
    ...
    // TODO
    cb(null, '', function(tan, cb2) {
      if (tan !== '1234') {
        cb2(new Error('falsche TAN'))
      } else cb2(null, 'TAN korrekt')
    })
  }
